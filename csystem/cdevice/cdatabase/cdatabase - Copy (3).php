...
class CTableMemory extends CMemory{
		...
		for($row=0; ($rowdata = mysql_fetch_assoc($result)); $row++) {
    		for($column=0; $column<$num_columns; $column++) {
				...
				if($table_name != "") {
					...
					$address="host,$database_name,$table_name,$column_name,$primary_key_name='$primary_key_value'";
				} // end if
				...
				//print_r($address);
				printbr();
				print_r($this->getParamsFromAddress($address));
				printbr();
			} // end for cols
		} // end while rows	
	...
	public function get( $strname ) {
		return ($this->m_table && isset($this->m_table[$strname])) ? $this->m_table[$strname] : ""; 
	} // end get)
	
	public function set( $strname, $value ) {
		if($this->m_table && isset($this->m_table[$strname])) 
			$this->m_table[$strname] = $value;
	} // end set()
	
	// create() / retrieve() / update() / delete()
	//[table][field=], NULL, NUMBER
	//public function create( $strname, $value, $strtype ){ 
	//	$row = array( 'm_strname'=>$strname, 'm_value'=>$value, 'm_strtype'=>$strtype ); 
	//	$value = $this->serialize( $value ); 
	//	return mysql_query( "INSERT INTO {$this->m_strtable} VALUES( '$strname', '$value', '$strtype' )", $this->m_connection ) ? $row : NULL; 
	//} // end create()
	...
	public function getParamsFromAddress($straddress) {
		$params = explode(",", $straddress);
		
		print_r($params);
		$host = isset($params[0]) ? $params[0] : "";
		$database = isset($params[1]) ? $params[1] : "";
		$table = isset($params[2]) ? $params[2] : "";
		$column = isset($params[3]) ? $params[3] : "";
		$pkname="";
		$pkvalue="";
		if(isset($params[4])) {
			$params = explode("=", $params[4]);
			$pkname = $params[0];
			$pkvalue = $params[1];
		} // end if		
		
		return array("host"=>$host, "database"=>$database, "table" => $table, "column" => $column, "pk-name" => $pkname, "pk-value" => $pkvalue);  
	} // end getParamsFromAddress()
