...
class CString {
	...
	public function match( $strregexp ){ 
		...
		if( $ilastg < $ilastslash )
			return (preg_match($strregexp->valueOf(),$this->m_str,$arrstrmatches)<=0) ? NULL : $arrstrmatches; 		
		$strafterg = $strregexp->substr( $ilastg + 1 );
		$strbeforeg = $strregexp->substring( 0, $ilastg );
		$strregexp = $strbeforeg->valueOf() . $strafterg->valueOf();
		return (preg_match_all($strregexp,$this->m_str,$arrstrmatches)<=0) ? NULL : $arrstrmatches; 
	} // end match()
